...
def generateDict(text):
    ...
    if len(words) > 2:
        ...
        for i, w in enumerate(words):
            firstWord = words[i]
            try:    
                #try to get the other words, maybe i am at the end of the list
                secondWord = words[i + 1]
                thirdWord = words[i + 2]
            except Exception: 
                #we don't care if we don't have words following the current one and then we exit the loop
                break

            key = (firstWord, secondWord)

            if key not in dictionary:   
                #this pair of words is new to the dictionary, and so doesn't have a list of following words
                dictionary[key] = []
...
def generateText(dictionary):
    ...
    while True:
        try:
            ...
            if DEBUG and len(possibleWords) > 1:   
                #with DEBUG enabled you will see a colored word when the word was chosen from a more-than-1 choice list, aka the sentence could have gone 2 or more ways.
                sentence.append("\033" + chosenWord + "\033")
                print("\'" + chosenWord + "\' from " + str(possibleWords))
            else:
                sentence.append(chosenWord)

            if chosenWord[-1] in END_PUNCTATION:    
                #if the last char of the word is a end sentence thing
                sentence.append("\n")
            ...
            #the path is closed, better start another sentence!
...
def generate_democrat_sentence():
    return pipeline.generateSpeech('Democrat')
